...
class FileUploadResponseDto {
  @ApiProperty({ description: 'URL to access the uploaded file', example: 'https://cdn.example.com/project-bucket/path/to/file.pdf' })
  url: string;

  @ApiProperty({ description: 'Original filename of the uploaded file', example: 'file.pdf' })
  filename: string;

  @ApiProperty({ description: 'File path within the storage bucket', example: 'path/to/file.pdf' })
  filepath: string;

  @ApiProperty({ description: 'MIME type of the uploaded file', example: 'application/pdf' })
  mimetype: string;

  @ApiProperty({ description: 'Size of the uploaded file in bytes', example: 102400 })
  size: number;
}

class FileUrlResponseDto {
  @ApiProperty({ description: 'A URL to access the file', example: 'https://cdn.example.com/project-bucket/path/to/file.pdf' })
  url: string;

  @ApiPropertyOptional({ description: 'Expiration date/time for the URL (if applicable)', example: '2024-06-15T10:00:00Z' })
  expiresAt?: string;
}

class FileUpdateResponseDto {
  @ApiProperty({ description: 'Indicates if the file update was successful', example: true })
  success: boolean;

  @ApiPropertyOptional({ description: 'List of fields that were updated', example: ['filename', 'filepath'] })
  updatedFields?: string[];
}

class FileDeleteResponseDto {
  @ApiProperty({ description: 'Indicates if the file deletion was successful', example: true })
  success: boolean;

  @ApiPropertyOptional({ description: 'Optional message describing the result of the deletion', example: 'File deleted successfully.' })
  message?: string;
}
...
export class FileController {
  ...
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        ...Object.assign({}, ...Object.entries(CreateFileDto.prototype).map(([key]) => ({
          [key]: { type: 'string' }
        }))),
        file: {
          type: 'string',
  ...
  @ApiQuery({ name: 'filepath', required: true, type: String, description: 'Path of the file in the bucket' })
  @ApiQuery({ name: 'project', required: true, type: String, description: 'Name of the storage bucket/project' })
  @ApiOkResponse({ 
  ...
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        ...Object.assign({}, ...Object.entries(UpdateFileDto.prototype).map(([key]) => ({
          [key]: { type: 'string' }
        }))),
        file: {
          type: 'string',
  ...
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        filepath: { type: 'string', description: 'Path of the file in the bucket' },
        project: { type: 'string', description: 'Name of the storage bucket/project' },
      },
  ...
  @ApiResponse({ status: 400, description: 'filepath or bucketName is required to delete a file.' })
...
constructor(private readonly supabaseService: SupabaseService){}
  ...
  @Get(':bucketName')
  @ApiOperation({ summary: 'List all files and folders in a specific bucket (project)' })
  @ApiParam({ name: 'bucketName', description: 'The name of the bucket/project' })
  ...
  @ApiOperation({ summary: 'List all files and folders within a folder in a specific bucket (project)' })
  @ApiParam({ name: 'bucketName', description: 'The name of the bucket/project' })
  @ApiParam({ name: 'folder', description: 'Path to the folder inside the bucket', required: true, type: 'string' })
  @Get(':bucketName/folder')
async listElementsInFolder(
  @Param('bucketName') bucketName: string,
  @Query('path') folderPath: string,
) {
  if (!folderPath) throw new BadRequestException('folder path query param is required');
